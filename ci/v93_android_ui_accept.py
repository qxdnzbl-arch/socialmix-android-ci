...
def find_node(text=None, desc=None, timeout=15):
    ...
    raise AssertionError(
        ...
        f"LOGCAT={adb('logcat', '-d', '-t', '220', check=False, text=True)[-12000:]}"
    )
...
def assert_fixture_exists():
    rows = adb(
        "shell", "content", "query",
        "--uri", "content://media/external_primary/audio/media",
        "--projection", "_id:title:artist:is_music:duration",
        check=False, text=True,
    )
    if TITLE not in rows:
        raise AssertionError(f"Seeded MediaStore audio missing before app flow: {rows}")
