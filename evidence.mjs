...
const INDEX_HTML = fs.readFileSync(new URL('./evidence/index.html', import.meta.url), 'utf8');
...
export async function handleEvidence(req,res){
  const u=new URL(req.url||'/evidence', `http://${req.headers.host||'localhost'}`);
  const pathname = u.pathname.startsWith('/evidence') ? (u.pathname.slice('/evidence'.length) || '/') : u.pathname;
  if (req.method==='OPTIONS') { res.writeHead(204,{'access-control-allow-origin':'*','access-control-allow-methods':'GET,POST,OPTIONS','access-control-allow-headers':'content-type'}); return res.end(); }
  if (pathname==='/health') return json(res,200,{ok:true,service:'拿证据'});
  if (pathname.startsWith('/event/')) {
    const name=decodeURIComponent(pathname.slice('/event/'.length));
  ...
  if (pathname==='/api/status') return json(res,200,{ok:true,search:['DuckDuckGo HTML','Wikipedia','Bing News RSS'],synthesis:'Pollinations + validated source IDs',payment:'not_enabled'});
