...
base = "4. After each result, choose one best next action. Avoid pointless repeated searches.\n"
rule = "4a. Optimize for result quality, not minimum spend. Use paid reasoning when it materially improves the result, but never spend tokens repeating unchanged analysis or retrying the same failed path without new evidence. Prefer deterministic/free execution tools when they can do the job, and verify outputs before another paid reasoning call.\n"

# Collapse duplicates created by older versions, then install exactly one copy.
while rule + rule in s:
    s = s.replace(rule + rule, rule)
if rule not in s:
    if base not in s:
        raise SystemExit('quality-first insertion marker not found')
    s = s.replace(base, base + rule, 1)

if "effective_daily_calls=max(LLM_MAX_CALLS_PER_DAY, 60)" not in s:
    s = s.replace(
...
if "effective_interval=min(LLM_MIN_CALL_INTERVAL_SECONDS, 120.0)" not in s:
    s = s.replace(
...
if "BUDGET_ADVISORY spent_today_cny=" not in s:
    s = s.replace(
...
p.write_text(s)
print('quality-first spending policy present exactly once')
