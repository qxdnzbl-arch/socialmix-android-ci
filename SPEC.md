...
## 0.5 UI Source Of Truth
- 2025 年版本基线：App Store 可话 1.13.3（2025-08-25）。
- 页面视觉以该 App Store 的 4 张公开截图为第一证据，并以 2024-11 同版 Android 公开截图交叉验证。
- 首页 App 内部只保留日期、`此刻，说你想说的话～`、大号白色输入卡与底部三枚图标；“从发动态开始 / 自由表达你的想法或感受”属于商店宣传图外层，不得做进 App。
- 底部导航只显示图标，不显示“首页 / 消息 / 我的”文字。
- 消息页使用白底、正常字号标题、顶部好友头像横排、紧凑聊天列表；不使用大标题+大面积空白。
- 共鸣页使用全屏白色内容页、左上关闭、右上序号、底部“点亮”；点亮后进入聊天。
- 未被公开证据确认的页面不得自行设计成其他社交产品常见样式并声称是原版。
